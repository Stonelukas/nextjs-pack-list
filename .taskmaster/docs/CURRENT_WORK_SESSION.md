...
- [x] The current complete client gate passes all 382 tests across 88 files. Vitest uses one file worker plus bounded 10-second Testing Library waits and a 15-second per-test ceiling so lazy-route transforms remain deterministic on constrained runners without retries.
