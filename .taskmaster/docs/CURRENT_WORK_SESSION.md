...
- [x] The current complete client gate passes all 380 tests across 88 files. Vitest uses one file worker plus bounded 10-second Testing Library waits and a 15-second per-test ceiling so lazy-route transforms remain deterministic on constrained runners without retries.
...
- [x] The current complete Convex gate passes 142 tests across eight files, including 23 focused administrator authorization/pagination tests and the deployable-module-path check. Strict typecheck passes. Full ESLint passes with 0 errors and 20 existing warnings. The current Vite/PWA production build generates a 69-entry shell-only precache and retains the existing >500 kB advisory.
