...
- Client Vitest mounts real routes/pages/components/hooks and replaces only external Clerk/Convex/PWA/Vercel edges. Its memory-router helpers invoke the production `AppProviders` and can select configured or unconfigured runtime results. The client project uses one file worker with bounded async/test timeouts; the current gate is 383 tests across 88 files.
